...
async function generatePoseidonInputs(count, outputDir, includeInvalid = false) {
...
async function main() {
  ...
  const outputDir = process.argv[5] || path.join(CIRCUITS_DIR, 'test-inputs', circuit);
  
  if (!circuit) {
    console.error('Usage: node generate-test-inputs.cjs <circuit> [count] [includeInvalid] [output-dir]');
    console.error('Example: node generate-test-inputs.cjs poseidon_test 10 true');
    process.exit(1);
  }
