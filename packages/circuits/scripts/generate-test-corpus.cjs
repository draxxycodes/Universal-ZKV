...
async function generateTestCorpus() {
  ...
  const totalValid = results.poseidon.valid + results.eddsa.valid + results.merkle.valid;
  const totalInvalid = results.poseidon.invalid + results.eddsa.invalid + results.merkle.invalid;
  const totalProofs = results.poseidon.proofs + results.eddsa.proofs + results.merkle.proofs;
  ...
  log('─────────────────────────────────────────────────────────');
  log(`Poseidon:  ${results.poseidon.valid} valid + ${results.poseidon.invalid} invalid = ${results.poseidon.proofs} proofs`, 'white');
  log(`EdDSA:     ${results.eddsa.valid} valid + ${results.eddsa.invalid} invalid = ${results.eddsa.proofs} proofs`, 'white');
  log(`Merkle:    ${results.merkle.valid} valid + ${results.merkle.invalid} invalid = ${results.merkle.proofs} proofs`, 'white');
  log('─────────────────────────────────────────────────────────');
  log(`Total:     ${totalValid} valid + ${totalInvalid} invalid = ${totalProofs} proofs`, 'green');
  ...
  if (totalProofs >= 600) {
    ...
    log(`⚠️  Target not fully met: ${totalProofs}/600 proofs generated`, 'yellow');
  }
...
function generateCatalog(results) {
  const catalog = {
    ...
    version: '1.0.0',
    summary: {
      totalInputs: results.poseidon.valid + results.poseidon.invalid +
                   results.eddsa.valid + results.eddsa.invalid +
                   results.merkle.valid + results.merkle.invalid,
      totalProofs: results.poseidon.proofs + results.eddsa.proofs + results.merkle.proofs,
      validInputs: results.poseidon.valid + results.eddsa.valid + results.merkle.valid,
      invalidInputs: results.poseidon.invalid + results.eddsa.invalid + results.merkle.invalid
    },
    circuits: {
      poseidon_test: {
        constraints: 601,
    ...
    usage: {
      ...
      batchVerify: 'find proofs/plonk/<circuit> -name proof.json -exec node scripts/plonk-cli.cjs verify <circuit> {} \\;',
