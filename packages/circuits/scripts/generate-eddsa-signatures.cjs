...
async function generateEdDSASignatures(count, outputDir, includeInvalid = false) {
  ...
  if (includeInvalid) {
    ...
    for (let i = 1; i <= invalidCount; i++) {
      ...
      if (invalidCase === 0) {
        ...
        // Tampered signature
        const tamperedS = (BigInt(signature.S.toString()) + BigInt(1)).toString();
...
async function main() {
  ...
  const includeInvalid = process.argv[3] === 'true' || process.argv[3] === '--invalid';
  const outputDir = process.argv[4] || path.join(CIRCUITS_DIR, 'test-inputs', 'eddsa_verify');
