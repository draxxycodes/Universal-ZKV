...
function ensureSetup(circuit) {
  ...
  if (!fs.existsSync(zkeyPath) || !fs.existsSync(vkPath)) {
    error(
      `PLONK setup not found for ${circuit}. Run: ./generate-plonk-proofs.sh`,
    );
  }
...
function generateWitness(circuit, inputPath, uniqueId) {
  ...
  if (!fs.existsSync(wasmPath)) {
    error(
      `WASM not found for ${circuit}. Compile first: circom src/${circuit}.circom --wasm`,
    );
  }
...
function generateSampleInputs(circuit, outputDir) {
  ...
  switch (circuit) {
    case "poseidon_test":
      ...
      log(
        `Note: expectedHash set to "0" - compute actual hashes for valid proofs`,
        "yellow",
      );
      break;
