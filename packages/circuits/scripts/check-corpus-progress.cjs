...
function countInputs(circuitDir) {
  ...
  return files.filter((f) => f.startsWith("input_") && f.endsWith(".json"))
    .length;
}
...
function main() {
  ...
  circuits.forEach((circuit) => {
    ...
    console.log(
      `  ${status} ${circuit.padEnd(20)} ${formatProgress(count, 250)}`,
    );
  });
  ...
  if (totalProofs === 750) {
    ...
    console.log(
      "  1. Verify corpus: node scripts/plonk-cli.cjs verify poseidon_test proofs/plonk/poseidon_test/batch/proof_1/proof.json",
    );
