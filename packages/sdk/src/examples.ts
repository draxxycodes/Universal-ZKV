...
console.log(`  Merkle Root: ${Buffer.from(statement.merkleRoot).toString('hex').slice(0, 16)}...`);
console.log(`  Public Key: ${Buffer.from(statement.publicKey).toString('hex').slice(0, 16)}...`);
console.log(`  Nullifier: ${Buffer.from(statement.nullifier).toString('hex').slice(0, 16)}...`);
...
console.log(`  First 32 bytes (merkle_root): ${Buffer.from(encodedStatement.slice(0, 32)).toString('hex')}`);
console.log(`  Next 32 bytes (public_key): ${Buffer.from(encodedStatement.slice(32, 64)).toString('hex')}`);
console.log(`  Next 32 bytes (nullifier): ${Buffer.from(encodedStatement.slice(64, 96)).toString('hex')}`);
console.log(`  Next 16 bytes (value u128): ${Buffer.from(encodedStatement.slice(96, 112)).toString('hex')}`);
console.log(`  Next 4 bytes (extra_len): ${Buffer.from(encodedStatement.slice(112, 116)).toString('hex')}`);
console.log(`  Extra data: ${Buffer.from(encodedStatement.slice(116)).toString('hex')}\n`);
...
console.log(`  Extra matches: ${Buffer.from(decodedStatement.extra).toString('hex') === Buffer.from(statement.extra).toString('hex')}\n`);
...
console.log(`  VK Hash: ${Buffer.from(groth16Proof.vkHash).toString('hex').slice(0, 16)}...`);
console.log(`  Proof size: ${groth16Proof.proofBytes.length} bytes`);
console.log(`  Public inputs size: ${groth16Proof.publicInputsBytes.length} bytes`);
...
console.log(`    proof_type: ${encodedProof[1]} (${ProofType[encodedProof[1]]})`);
console.log(`    program_id: ${encodedProof[2]} ${encodedProof[3]} ${encodedProof[4]} ${encodedProof[5]}`);
console.log(`    vk_hash: ${Buffer.from(encodedProof.slice(6, 38)).toString('hex').slice(0, 16)}...`);

// Extract proof_len (u32 little-endian at offset 38)
const proofLen = encodedProof[38] | (encodedProof[39] << 8) | (encodedProof[40] << 16) | (encodedProof[41] << 24);
...
const publicInputsLen = encodedProof[publicInputsOffset] | (encodedProof[publicInputsOffset + 1] << 8) |
                         (encodedProof[publicInputsOffset + 2] << 16) | (encodedProof[publicInputsOffset + 3] << 24);
...
console.log(`  Matches original: ${decodedProof.proofType === groth16Proof.proofType && decodedProof.programId === groth16Proof.programId}\n`);
...
console.log(`  Merkle Root: ${Buffer.from(extractedStatement.merkleRoot).toString('hex').slice(0, 16)}...`);
console.log(`  Value: ${extractedStatement.value}`);
console.log(`  Nullifier: ${Buffer.from(extractedStatement.nullifier).toString('hex').slice(0, 16)}...\n`);
...
console.log(`  Proof size: ${starkProof.proofBytes.length.toLocaleString()} bytes`);
console.log(`  Total encoded size: ${starkProof.encodedSize().toLocaleString()} bytes\n`);
...
console.log(`  Call: verifier.verify(0x${Buffer.from(onChainBytes).toString('hex')})`);
console.log(`  Gas estimate: ~280,000 (Groth16) / ~400,000 (PLONK) / ~540,000 (STARK)`);
