...
describe('Circuit Helpers', () => {
  describe('fieldElementToBytes32', () => {
    ...
    it('should handle large field elements', () => {
      const field = "8234104122482341265491137074636836252947884782870784360943022469005013929455";
      const bytes = fieldElementToBytes32(field);
  ...
  describe('parseSnarkjsPublicInputs', () => {
    it('should parse 5 public inputs into PublicStatement format', () => {
      ...
      expect(inputs.public_key).toBe("8234104122482341265491137074636836252947884782870784360943022469005013929455");
      expect(inputs.nullifier).toBe("15046301437886485981820688550832103193484625835926133964909148811342873885668");
  ...
  describe('circuitInputsToPublicStatement', () => {
    it('should convert circuit inputs to PublicStatement', () => {
      const inputs = {
        merkle_root: "0",
        public_key: "8234104122482341265491137074636836252947884782870784360943022469005013929455",
        nullifier: "15046301437886485981820688550832103193484625835926133964909148811342873885668",
        value: "1000000",
        extra: "0"
      };
  ...
  describe('createGroth16Proof', () => {
    it('should create UniversalProof from circuit outputs', () => {
      ...
      const proof = createGroth16Proof("poseidon_with_statement", publicInputs, proofBytes, vkHash);
  ...
  describe('createProofPayload', () => {
    it('should create complete proof payload', () => {
      ...
      const vkHash = new Uint8Array(32);
      const payload = createProofPayload("poseidon_with_statement", publicInputs, proofJson, vkHash);
    ...
    it('should throw on invalid PublicStatement', () => {
      ...
      expect(() => createProofPayload(1, publicInputs, proofJson, vkHash)).toThrow();
    });
  });
