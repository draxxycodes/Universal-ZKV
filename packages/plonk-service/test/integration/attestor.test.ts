...
describe("Attestor Integration Tests", () => {
  ...
  describe("GET /attestation/:proofHash", () => {
    it("should retrieve attestation status by proof hash", async () => {
      ...
      const verifyResponse = await request(app).post("/verify").send({
        circuitType: "poseidon_test",
        proof,
        publicSignals,
        submitAttestation: true,
  ...
  describe("GET /attestation/events", () => {
    ...
    it("should paginate results", async () => {
      ...
      expect(response1.body.events).toHaveLength(
        Math.min(5, response1.body.total || 5),
      );

      if (
        response1.body.events.length > 0 &&
        response2.body.events.length > 0
      ) {
        expect(response1.body.events[0].proofHash).not.toBe(
          response2.body.events[0].proofHash,
        );
      }
    });
