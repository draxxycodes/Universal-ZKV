...
describe('End-to-End Verification Flow', () => {
  ...
  describe('Complete Poseidon Workflow', () => {
    it('should generate input, create proof, and verify', async () => {
      ...
      const inputPath = join(CIRCUITS_PATH, 'test-inputs/poseidon_test/input_1.json');
      ...
      const proof = JSON.parse(await readFile(join(outputPath, 'proof.json'), 'utf-8'));
      const publicSignals = JSON.parse(await readFile(join(outputPath, 'public.json'), 'utf-8'));
  ...
  describe('Complete EdDSA Workflow', () => {
    it('should generate signature, create proof, and verify', async () => {
      ...
      const inputPath = join(CIRCUITS_PATH, 'test-inputs/eddsa_verify/input_1.json');
  ...
  describe('Complete Merkle Workflow', () => {
    it('should generate Merkle proof, create PLONK proof, and verify', async () => {
      ...
      // Step 2: Generate PLONK proof
      const inputPath = join(CIRCUITS_PATH, 'test-inputs/merkle_proof/input_1.json');
  ...
  describe('Batch Workflow', () => {
    it('should generate and verify batch of proofs', async () => {
      ...
      const proofs = [];
      for (let i = 1; i <= 5; i++) {
        const inputPath = join(CIRCUITS_PATH, `test-inputs/poseidon_test/input_${i}.json`);
  ...
  describe('Performance Benchmarks', () => {
    it('should benchmark full workflow performance', async () => {
      ...
      // Calculate averages
      const avgInputGen = timings.inputGeneration.reduce((a, b) => a + b, 0) / iterations;
      const avgProofGen = timings.proofGeneration.reduce((a, b) => a + b, 0) / iterations;
      const avgVerify = timings.verification.reduce((a, b) => a + b, 0) / iterations;
