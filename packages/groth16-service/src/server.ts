...
app.get('/', (_req: Request, res: Response) => {
  res.json({
    service: 'UZKV Groth16 Verification Service',
    version: '0.1.0',
    description: 'Off-chain Groth16 proof verification with on-chain attestation',
    endpoints: {
      verify: 'POST /verify',
    ...
    attestorContract: process.env.ATTESTOR_ADDRESS || '0x36e937ebcf56c5dec6ecb0695001becc87738177',
...
async function start() {
  try {
    logger.info('Initializing WASM verifier...');
    await wasmVerifier.initialize();
    logger.info('WASM verifier initialized successfully');
    ...
    app.listen(PORT, () => {
      logger.info(
        ...
        'Groth16 verification service started'
      );
    });
  } catch (error) {
