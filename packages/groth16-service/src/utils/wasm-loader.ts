...
class Groth16WasmVerifier {
  ...
  async verify(
    ...
    try {
      logger.info({ publicInputsCount: publicInputs.length }, 'Verifying Groth16 proof');
  ...
  private validateProof(proof: Groth16Proof): void {
    ...
    if (!proof.pi_a || (proof.pi_a.length !== 2 && proof.pi_a.length !== 3)) {
      throw new Error('Invalid proof: pi_a must be an array of 2 or 3 elements');
    }
    if (!proof.pi_b || !Array.isArray(proof.pi_b) || proof.pi_b.length < 2) {
      throw new Error('Invalid proof: pi_b must be an array with at least 2 elements');
    }
    if (!proof.pi_c || (proof.pi_c.length !== 2 && proof.pi_c.length !== 3)) {
      throw new Error('Invalid proof: pi_c must be an array of 2 or 3 elements');
    }
    if (proof.protocol !== 'groth16') {
      throw new Error(`Invalid protocol: expected 'groth16', got '${proof.protocol}'`);
    }
  ...
  private validateVerificationKey(vk: VerificationKey): void {
    if (vk.protocol !== 'groth16') {
      throw new Error(`Invalid VK protocol: expected 'groth16', got '${vk.protocol}'`);
    }
    if (vk.curve !== 'bn128') {
