...
async function generateValidProof() {
  try {
    ...
    console.log(`   Preimage: [${witness.preimage[0]}, ${witness.preimage[1]}]`);
    console.log(`   Hash: ${witness.expectedHash}\n`);
    
    // Step 2: Locate circuit files
    const buildDir = path.join(__dirname, '..', 'packages', 'circuits', 'build');
    const wasmFile = path.join(buildDir, 'poseidon_test_js', 'poseidon_test.wasm');
    ...
    try {
      ...
      console.error('❌ Circuit files not found. Please compile circuits first.');
