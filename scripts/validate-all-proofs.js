...
const sampleSize = args.includes("--sample")
  ...
  : null;
const targetCircuit = args.includes("--circuit")
...
function verifyProof(circuit, proofFile, type) {
  const proofPath = path
    .join(PROOFS_DIR, circuit, type, proofFile)
    .replace(/\\/g, "/");
  const publicFile = proofFile.replace("_proof.json", "_public.json");
  const publicPath = path
    .join(PROOFS_DIR, circuit, type, publicFile)
    .replace(/\\/g, "/");
...
function validateCircuit(circuit) {
  ...
  for (let i = 0; i < validProofs.length; i++) {
    ...
    if (result.success && result.valid) {
      ...
      console.log(
        `  ❌ UNEXPECTED: Valid proof failed verification: ${validProofs[i]}`,
      );
    } else {
    ...
    if ((i + 1) % validBatchSize === 0) {
      ...
      process.stdout.write(
        `  Progress: ${i + 1}/${validProofs.length} (${validProgress}%)\r`,
      );
    }
  }
  ...
  if (circuitResults.valid.unexpectedFailures > 0) {
    console.log(
      `  ❌ Unexpected failures: ${circuitResults.valid.unexpectedFailures}`,
    );
  }
  ...
  if (invalidProofs.length > 0) {
    ...
    for (let i = 0; i < invalidProofs.length; i++) {
      ...
      if (result.success && !result.valid) {
        ...
        console.log(
          `  ⚠️  CRITICAL: Invalid proof passed verification: ${invalidProofs[i]}`,
        );
      } else {
      ...
      if ((i + 1) % invalidBatchSize === 0) {
        ...
        process.stdout.write(
          `  Progress: ${i + 1}/${invalidProofs.length} (${invalidProgress}%)\r`,
        );
      }
    }
    ...
    if (circuitResults.invalid.unexpectedFailures > 0) {
      console.log(
        `  ⚠️  CRITICAL failures: ${circuitResults.invalid.unexpectedFailures}`,
      );
    }
  }
  ...
  results.summary.totalProofs +=
    circuitResults.valid.total + circuitResults.invalid.total;
  ...
  results.summary.successfulValidations +=
    circuitResults.valid.verified + circuitResults.invalid.verified;
  results.summary.expectedFailures += circuitResults.invalid.verified;
  results.summary.unexpectedFailures +=
    circuitResults.valid.unexpectedFailures +
    circuitResults.invalid.unexpectedFailures;
...
function generateReport() {
  ...
  console.log(`\nValidation Results:`);
  console.log(
    `  ✅ Successful validations: ${results.summary.successfulValidations}`,
  );
  console.log(
    `  ⚠️  Expected failures (invalid proofs): ${results.summary.expectedFailures}`,
  );
  console.log(
    `  ❌ Unexpected failures: ${results.summary.unexpectedFailures}`,
  );
  ...
  for (const circuit of Object.keys(results.circuits)) {
    ...
    if (cr.valid.unexpectedFailures > 0) {
      console.log(
        `      ❌ Unexpected failures: ${cr.valid.unexpectedFailures}`,
      );
    }
    console.log(
      `    Invalid: ${cr.invalid.verified}/${cr.invalid.total} correctly rejected`,
    );
    if (cr.invalid.unexpectedFailures > 0) {
      console.log(
        `      ⚠️  CRITICAL failures: ${cr.invalid.unexpectedFailures}`,
      );
    }
  }
  ...
  const successRate =
    totalExpected > 0
      ? ((totalSuccess / totalExpected) * 100).toFixed(2)
      : "0.00";
