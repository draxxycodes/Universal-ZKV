 ...
 */
async function verifyUniversalProof(
  proofType,
  proofPath,
  publicInputsPath,
  vkPath,
...
async function verifyGroth16(proofPath, publicInputsPath, vkPath) {
  ...
  const publicInputsData = JSON.parse(
    fs.readFileSync(publicInputsPath, "utf8"),
  );
...
async function verifySTARK(proofPath, publicInputsPath) {
  ...
  if (offset !== proofBytes.length) {
    throw new Error(
      `UniversalProof size mismatch: expected ${offset}, got ${proofBytes.length}`,
    );
  }
  ...
  console.log(
    `✅ STARK proof structure is valid (${proofLen} bytes proof, ${pubLen} bytes public inputs)`,
  );
  return true;
}
