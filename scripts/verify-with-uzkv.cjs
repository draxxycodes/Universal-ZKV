...
const proofOnlyFiles = proofFiles.filter(f => 
  (f.includes('_proof.json') || f.endsWith('_proof.ub'))
);
...
for (const proofFile of proofOnlyFiles) {
  ...
  if (result.success) {
    ...
    if (result.details) {
      console.log(`   📊 Version: ${result.details.version}, ProgramId: ${result.details.programId}`);
    }
    verifiedCount++;
  } else {
