...
// Configuration
const DEPLOYMENT_FILE = path.join(__dirname, '../deployments/sepolia-deployment.json');
...
function loadDeployment() {
  if (!fs.existsSync(DEPLOYMENT_FILE)) {
    console.error('❌ Deployment file not found. Run ./scripts/deploy-testnet.sh first');
    process.exit(1);
  }
...
async function benchmark() {
  ...
  console.log(`  Universal Verifier: ${deployment.contracts.universalZKVerifier}\n`);
  ...
  for (const [proofType, vectors] of Object.entries(TEST_VECTORS)) {
    ...
    try {
      // Register VK first
      const registerTx = await verifier.registerVerificationKey(typeId, vectors.vk);
      const registerReceipt = await registerTx.wait();
      
      // Measure verification gas
      const verifyTx = await verifier.verify(typeId, vectors.proof, vectors.publicInputs, vectors.vk);
      ...
      console.log(`  ${proofType.toUpperCase()}: ${gasUsed.toLocaleString()} gas`);
    } catch (error) {
  ...
  for (const size of batchSizes) {
    try {
      ...
      const batchTx = await verifier.batchVerify(0, proofs, inputs, TEST_VECTORS.groth16.vk);
      ...
      console.log(`  Batch ${size}: ${gasUsed.toLocaleString()} gas (${gasPerProof.toLocaleString()} per proof)`);
    } catch (error) {
  ...
  for (const [proofType, vectors] of Object.entries(TEST_VECTORS)) {
    ...
    try {
      ...
      const registerTx = await verifier.registerVerificationKey(typeId, uniqueVk);
      const registerReceipt = await registerTx.wait();
  ...
  try {
    ...
    for (let i = 0; i < iterations; i++) {
      const tx = await verifier.verify(0, TEST_VECTORS.groth16.proof, TEST_VECTORS.groth16.publicInputs, TEST_VECTORS.groth16.vk);
    ...
    console.log(`  Privacy App (5 sequential): ${totalGas.toLocaleString()} gas (${(totalGas / BigInt(iterations)).toLocaleString()} avg)`);
  } catch (error) {
  ...
  try {
    ...
    console.log(`  Rollup Aggregation (20 tx): ${gasUsed.toLocaleString()} gas (${(gasUsed / BigInt(txCount)).toLocaleString()} per tx)`);
  } catch (error) {
...
function generateReport() {
  ...
  report.push(`**Universal Verifier:** \`${results.contracts.universalZKVerifier}\``);
  report.push('');
  report.push('---');
  report.push('');
  ...
  results.benchmarks.single.forEach(b => {
    report.push(`| ${b.proofType.toUpperCase()} | ${parseInt(b.gasUsed).toLocaleString()} | ${b.proofSize} bytes | ${b.vkSize} bytes |`);
  });
  report.push('');
  ...
  const singleGas = results.benchmarks.single.find(b => b.proofType === 'groth16');
  if (singleGas) {
    results.benchmarks.batch.forEach(b => {
      const efficiency = ((1 - parseInt(b.gasPerProof) / parseInt(singleGas.gasUsed)) * 100).toFixed(2);
      report.push(`| ${b.batchSize} | ${parseInt(b.totalGas).toLocaleString()} | ${parseInt(b.gasPerProof).toLocaleString()} | ${efficiency}% |`);
    });
  }
  report.push('');
  ...
  results.benchmarks.registration.forEach(b => {
    report.push(`| ${b.proofType.toUpperCase()} | ${parseInt(b.gasUsed).toLocaleString()} | ${b.vkSize} bytes |`);
  });
  report.push('');
  ...
  results.benchmarks.workflows.forEach(b => {
    report.push(`| ${b.workflow} | ${parseInt(b.totalGas).toLocaleString()} | ${parseInt(b.avgGasPerOp || b.avgGasPerTx).toLocaleString()} |`);
  });
  report.push('');
  ...
  if (results.benchmarks.batch.length > 0) {
    ...
    if (batch10 && singleGas) {
      const savings = ((1 - parseInt(batch10.gasPerProof) / parseInt(singleGas.gasUsed)) * 100).toFixed(2);
      report.push(`- **Batch Efficiency:** ${savings}% gas savings for batch verification (10 proofs)`);
    }
  }
  
  if (results.benchmarks.workflows.length > 0) {
    const rollup = results.benchmarks.workflows.find(w => w.workflow.includes('Rollup'));
    if (rollup) {
      report.push(`- **Rollup Performance:** ${parseInt(rollup.avgGasPerTx).toLocaleString()} gas per transaction in batch of 20`);
    }
  }
  ...
  report.push('✅ **Batch verification provides significant efficiency gains**');
