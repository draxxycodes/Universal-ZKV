...
async function generateInvalidProof(circuit, id, method = null) {
  ...
  try {
    ...
    if (selectedMethod === "witness") {
      ...
      const tempProofPath = path.join(
        PROOFS_DIR,
        circuit,
        "valid",
        `${circuit}_temp_${id}_proof.json`,
      );
      const tempPublicPath = path.join(
        PROOFS_DIR,
        circuit,
        "valid",
        `${circuit}_temp_${id}_public.json`,
      );
      const tempWitnessPath = path.join(
        PROOFS_DIR,
        circuit,
        "valid",
        `${circuit}_temp_${id}_witness.json`,
      );
    ...
    const proofPath = path.join(
      ...
      circuit,
      "invalid",
      `${circuit}_invalid_${id}_proof.json`,
    );
    ...
    const publicPath = path.join(
      ...
      circuit,
      "invalid",
      `${circuit}_invalid_${id}_public.json`,
    );
    ...
    const metadataPath = path.join(
      ...
      circuit,
      "invalid",
      `${circuit}_invalid_${id}_metadata.json`,
    );
    fs.writeFileSync(
      metadataPath,
      JSON.stringify(
    ...
    console.error(
      `\n❌ Error generating invalid proof ${id} for ${circuit}:`,
      error.message,
    );
    stats.errorCount++;
    throw error;
...
async function generateInvalidProofsForCircuit(circuit, count) {
  ...
  const circuitTime = (
    (Date.now() - stats.byCircuit[circuit].startTime) /
    1000
  ).toFixed(2);
  console.log(`\n${"=".repeat(60)}`);
  console.log(
    `✅ ${circuit} complete: ${stats.byCircuit[circuit].count} invalid proofs in ${circuitTime}s`,
  );
  console.log(`${"=".repeat(60)}`);
}
...
async function generateInvalidProofDataset() {
  ...
  const count =
    args.length > 0 && !isNaN(args[0]) ? parseInt(args[0]) : DEFAULT_COUNT;
  ...
  console.log(
    `   Total invalid proofs: ${(count * selectedCircuits.length).toLocaleString()}`,
  );
  ...
  console.log(
    `   Total time: ${totalTime}s (${(parseFloat(totalTime) / 60).toFixed(2)} minutes)`,
  );
