...
async function generateProof(circuit, witness, id) {
  ...
  try {
    ...
    console.error(
      `\n❌ Error generating proof ${id} for ${circuit}:`,
      error.message,
    );
    throw error;
  }
...
async function generateProofsForCircuit(circuit, count) {
  ...
  console.log(`\n${"=".repeat(60)}`);
  console.log(
    `✅ ${circuit} complete: ${stats.byCircuit[circuit].count} proofs in ${circuitTime}s`,
  );
  console.log(`${"=".repeat(60)}`);
}
...
async function generateProofDataset() {
  ...
  console.log(
    `   Total proofs: ${(count * selectedCircuits.length).toLocaleString()}`,
  );
